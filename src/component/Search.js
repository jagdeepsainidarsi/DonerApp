...
return(
   ...
   <Link style={{textDecoration:"none"}} to="/"> <Button style={{margin:"10px"}} variant="contained">Home</Button></Link>
     
   
    <div id="head">
       <div id="textbox">
       <input  value={search.searchvalue} name="searchvalue" onChange={handleinput} placeholder="Search The Donner By State Name" id="input"type="text"/></div>
         <div id="bottom">
          <div id="over">
           <table>
               <thead>
    ...
    return (<tbody key={key}><tr><td>{val.first_name+" "+val.last_name}</td><td>{val.phoneno}</td><td>{val.address}</td><td>{val.distt}</td><td>{val.state}</td><td>{val.coronapositive.substring(0,10)}</td><td>{val.coronanegative.substring(0,10)}</td></tr></tbody>)})}
